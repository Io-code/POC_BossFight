...
public class OldBossInput : MonoBehaviour
{
    [Header("Target")]
    ...
    public void Move()
    {
        Vector3 velocity = MoveVec().normalized * speed;
        if(move)
    ...
    public float DistFromT_Offset()
    {
        return Vector3.Distance(transform.position, T_OffsetPos());
    }
    #endregion
    ...
    private void OnDrawGizmos()
    {
        ...
        if (!relativPos)
            Gizmos.DrawLine(target.position, T_OffsetPos());
